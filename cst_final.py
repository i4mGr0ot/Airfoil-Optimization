...
class PhysicCSTFitter:
    ...
    def objective_function(self, params, x_u, y_u, x_l, y_l):
        ...
        # Weight leading edge errors higher (critical for stall)
        weight_u = 1.0 + 5.0 * np.exp(-20 * x_u)
        weight_l = 1.0 + 5.0 * np.exp(-20 * x_l)
        
        residuals = np.concatenate((res_u * weight_u, res_l * weight_l))

        # 2. [NEW] Aerodynamic Smoothing (Regularization)
        # Penalize "jagged" weights (high 2nd derivative of weights)
        # This prevents "wavy" surfaces that confuse XFOIL
        reg_u = np.diff(np.diff(w_u)) * 0.05
        reg_l = np.diff(np.diff(w_l)) * 0.05

        # 3. [NEW] Kutta Condition Constraint
        # TE Slope must converge. 
        # Analytical Slope CST @ TE ~= -Last_Weight + dz/2
        slope_u_te = -w_u[-1] + dz/2.0
        slope_l_te = w_l[-1] - dz/2.0
        
        # Penalty if Upper slope is positive (diverging up) or Lower is negative (diverging down)
        kutta_penalty = []
    ...
    def fit_airfoil(self, upper_pts, lower_pts):
        ...
        # Initial Guess (NACA 0012-ish)
        initial_guess = np.ones(1 + 2*(self.n_params-1) + 1) * 0.15
        initial_guess[-1] = dz_est

        # Tight Bounds to enforce realistic airfoils
        # w0 (Nose) constrained to prevent sharp spikes
        lb = [0.1] + [-0.5]*14 + [0.0]
        ub = [0.6] + [ 0.8]*14 + [0.02] # Max TE thickness 2%
        ...
        # If the geometric error is too high, the smoothing constraints likely 
        # fought the geometry too hard -> The airfoil is garbage/noisy.
        if res.cost > 0.02: 
            status = "POOR_FIT"
        else:
            status = "SUCCESS"
