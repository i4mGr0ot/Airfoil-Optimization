...
def get_dataloaders(h5_path):
    ...
    # --- KEY CHANGE: Use MinMaxScaler for Targets? ---
    # No, StandardScaler is better for training dynamics, 
    # BUT we must handle the weight of CD in the Loss Function, not the scaler.
    scalar_scaler = StandardScaler()
    s_train_norm = scalar_scaler.fit_transform(s_train)
    s_val_norm = scalar_scaler.transform(s_val)
    ...
    return (
        ...
        DataLoader(val_ds, batch_size=CONFIG['BATCH_SIZE'], shuffle=False, drop_last=True)
    )

# --- COMPACT NETWORK (Prevents Overfitting) ---
...
class ReAwareAirfoilPointNet(nn.Module):
    def __init__(self):
        ...
        # SLIGHTLY REDUCED CAPACITY TO FORCE GENERALIZATION
        # [64,64,128] -> [32,64,128]
        self.sa1 = PointNetSetAbstraction(npoint=100, radius=0.1, nsample=32, in_channel=0, mlp=[32, 64, 128], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=32, radius=0.2, nsample=32, in_channel=128, mlp=[64, 128, 256], group_all=False)
...
def physics_loss(s_pred, s_target, cp_pred, cp_target):
    """
    Weighted Loss to enforce Drag precision.
    s_target cols: 0=CL, 1=CD, 2=CM
    """
    # 1. Huber Loss (SmoothL1) is better for tiny aerodynamic coefficients
    criterion = nn.SmoothL1Loss()
    
    loss_cl = criterion(s_pred[:,0], s_target[:,0])
    loss_cd = criterion(s_pred[:,1], s_target[:,1])
    loss_cm = criterion(s_pred[:,2], s_target[:,2])
    
    loss_cp = F.mse_loss(cp_pred, cp_target)
    
    # 2. HEAVY WEIGHT on Drag (CD)
    # CD is 100x smaller than CL, so we multiply its loss by 100 to balance gradients.
    total_loss = (1.0 * loss_cl) + (100.0 * loss_cd) + (1.0 * loss_cm) + (5.0 * loss_cp)
...
def train():
    ...
    print(f"\n--- Starting High-Precision Training on {CONFIG['DEVICE']} ---")
    for epoch in range(CONFIG["EPOCHS"]):
        ...
        for pc, re_in, s_target, cp_target in pbar:
            ...
            loss = physics_loss(s_pred, s_target, cp_pred, cp_target)
            
            if torch.isnan(loss):
                print("CRITICAL: NaN Loss. Reducing LR.")
                break
        ...
        if avg_val < best_loss:
            ...
            print(f"    >>> High-Precision Model Saved (Val: {avg_val:.5f})")
