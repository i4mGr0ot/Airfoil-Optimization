...
# [Standard CST Kernel implementation for consistency]
...
# --- 2. PHYSICS-INFORMED DATA PIPELINE ---
def get_dataloaders(h5_path):
    if not os.path.exists(h5_path):
        print(f"CRITICAL: {h5_path} missing."); sys.exit(1)
    ...
    # Filter Invalid Physics
    # Cd must be positive for Log Transform
    valid_mask = np.isfinite(scalars).all(axis=1) & (scalars[:, 1] > 1e-6)
    weights = weights[valid_mask]
    scalars = scalars[valid_mask]
    cp = cp[valid_mask]

    print(f"    Kept {len(weights)} physically valid samples.")

    # --- KEY CHANGE: LOG-SPACE TRANSFORMATION ---
    # Lift (CL) -> Standard Scale (Linear)
    # Drag (CD) -> Log Scale (Order of Magnitude)
    # Moment (CM) -> Standard Scale (Linear)
    # Reynolds -> Log Scale (Physics standard)
    
    # 1. Transform Reynolds
    re_raw = scalars[:, 4].reshape(-1, 1)
    re_log = np.log10(re_raw) # Log-Reynolds is the industry standard input
    
    # 2. Transform Targets
    cl_raw = scalars[:, 0].reshape(-1, 1)
    cd_raw = scalars[:, 1].reshape(-1, 1)
    cm_raw = scalars[:, 2].reshape(-1, 1)
    
    cd_log = np.log10(cd_raw) # This makes 0.005 -> -2.3, balancing it with CL ~ 0.8
    
    # Combine Targets: [CL, Log10(CD), CM]
    targets_physics = np.hstack((cl_raw, cd_log, cm_raw))
    ...
    # Split
    X_pc = point_clouds
    y_targets = targets_physics
    y_re = re_log 
    y_cp = cp

    pc_train, pc_val, t_train, t_val, re_train, re_val, cp_train, cp_val = train_test_split(
        X_pc, y_targets, y_re, y_cp, test_size=0.1, random_state=42, shuffle=True
    )

    # --- SCALING ---
    # We apply StandardScaler to the TRANSFORMED values to center them at 0
    # This gives the Neural Network the easiest possible math problem.
    target_scaler = StandardScaler()
    t_train_norm = target_scaler.fit_transform(t_train)
    t_val_norm = target_scaler.transform(t_val)
    
    # Save scaler statistics for inference later
    import joblib
    joblib.dump(target_scaler, 'physics_scaler.pkl')

    re_scaler = StandardScaler()
    re_train_norm = re_scaler.fit_transform(re_train)
    re_val_norm = re_scaler.transform(re_val)

    # Tensor Conversion
    train_ds = TensorDataset(
        torch.tensor(pc_train, dtype=torch.float32),
        torch.tensor(re_train_norm, dtype=torch.float32),
        torch.tensor(t_train_norm, dtype=torch.float32),
    ...
    val_ds = TensorDataset(
        ...
        torch.tensor(t_val_norm, dtype=torch.float32),
        torch.tensor(cp_val, dtype=torch.float32)
    )

    return DataLoader(train_ds, batch_size=CONFIG['BATCH_SIZE'], shuffle=True, drop_last=True), DataLoader(val_ds, batch_size=CONFIG['BATCH_SIZE'], shuffle=False, drop_last=True)

# --- 3. ARCHITECTURE (Standard 64-width) ---
...
def train():
    ...
    print(f"\n--- Training with Log-Space Physics ---")
    for epoch in range(CONFIG["EPOCHS"]):
        model.train()
        train_loss = 0.0
        
        for pc, re_in, t_target, cp_target in tqdm(train_loader, desc=f"Ep {epoch+1}", leave=False):
            pc, re_in = pc.to(CONFIG["DEVICE"]), re_in.to(CONFIG["DEVICE"])
            t_target, cp_target = t_target.to(CONFIG["DEVICE"]), cp_target.to(CONFIG["DEVICE"])
            
            optimizer.zero_grad()
            s_pred, cp_pred = model(pc, re_in)
            
            # Loss is simple MSE now because data is naturally balanced by log-scale
            loss = F.mse_loss(s_pred, t_target) + F.mse_loss(cp_pred, cp_target)
        ...
        val_loss = 0.0
        with torch.no_grad():
            for pc, re_in, t_target, cp_target in val_loader:
                pc, re_in = pc.to(CONFIG["DEVICE"]), re_in.to(CONFIG["DEVICE"])
                t_target, cp_target = t_target.to(CONFIG["DEVICE"]), cp_target.to(CONFIG["DEVICE"])
                s_pred, cp_pred = model(pc, re_in)
                val_loss += (F.mse_loss(s_pred, t_target) + F.mse_loss(cp_pred, cp_target)).item()
        ...
        if avg_val < best_loss:
            best_loss = avg_val
            torch.save(model.state_dict(), "best_log_physics_pointnet.pth")
            print(f"    >>> Model Saved (Log-Space Validated)")
