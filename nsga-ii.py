...
CONFIG = {
    ...
    "OUTPUT_H5": "airfoil_dataset_nsga2.h5",
    "N_CORES": 6,
    "TARGET_SAMPLES": 7500,     # Total valid samples to save before stopping
    "POPULATION_SIZE": 100,     # Size of the active evolutionary population
    
    # Physics Settings
    "REYNOLDS": 600000,
    "MACH": 0.0,
    "ALPHA_SEQ": (-2.0, 16.0, 0.5), 
    "ITERATIONS": 200,
    "XFOIL_TIMEOUT": 30.0,
    ...
    # Aerodynamic Constraints (Hard limits for validity)
    "CL_MAX_LIMIT": 2.0,   
    "CD_MIN_LIMIT": 0.004,  
}
...
class RobustCST:
    ...
    def generate(self, w_u, w_l, dz_te):
        ...
        if np.any((y_u - y_l) < -1e-6): return None, None, "INVALID: Geometry Cross"
        
        max_t = np.max(y_u - y_l)
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        try:
            ...
            if len(polar_data) > 5 and best_stats['ld'] > 5.0:
                ...
                # --- NSGA-II OBJECTIVES ---
                # We MAXIMIZE all of these in the sorting logic.
                # Obj 1: Efficiency (L/D)
                obj_ld = best_stats['ld']
                
                # Obj 2: Operational Range (Bucket Width)
                obj_width = bucket_width
                
                # Obj 3: Stability (Minimize |Cm| -> Maximize -|Cm|)
                # Closer to 0 is better. -0.1 is better than -0.2.
                obj_stability = -abs(best_stats['cm'])
                ...
                subprocess.run(XFOIL_PATH, input=cmds_cp, text=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=10.0, cwd=tmp_dir)
                ...
                result = {
                    ...
                    'cp': cp_fixed,
                    # THE OBJECTIVE VECTOR [L/D, Width, -|Cm|]
                    'objectives': [obj_ld, obj_width, obj_stability]
                }
        except Exception: pass    
    return result

# --- NSGA-II ALGORITHM CORE ---
class NSGA2_Core:
    @staticmethod
    def dominates(p_objs, q_objs):
        """Returns True if p dominates q."""
        # We assume MAXIMIZATION for all objectives.
...
class GeneticEngine:
        ...
        try:
            ...
            for _, row in df.iterrows():
                self.population.append({
                    ...
                    'objectives': None # Evaluated later
                })
    ...
    def save_batch(self, h5f, batch, datasets):
        ...
        # [CL, CD, CM, Alpha, L/D, Width]
        s = np.array([[r['cl'], r['cd'], r['cm'], r['alpha_opt'], r['objectives'][0], r['objectives'][1]] for r in batch])
    ...
    def run(self):
        ...
        with h5py.File(CONFIG['OUTPUT_H5'], 'w') as h5f:
            ...
            # We only evaluate a subset of seeds to save time if DB is huge
            limit_init = min(len(tasks), CONFIG['POPULATION_SIZE'] * 2) 
            results = pool.map(worker_task, tasks[:limit_init])
            
            valid_pop = [r for r in results if r is not None]
            if not valid_pop: print("Seeds invalid."); return
            ...
            for f in fronts:
                if len(self.population) + len(f) <= CONFIG['POPULATION_SIZE']:
                    ...
                    needed = CONFIG['POPULATION_SIZE'] - len(self.population)
                    self.population.extend(f[:needed])
                    break
            ...
            print(f"\nSTARTING NSGA-II LOOP | Target: {CONFIG['TARGET_SAMPLES']}")
            
            while total_saved < CONFIG['TARGET_SAMPLES']:
                gen += 1
                offspring_tasks = []
                
                # Generate Offspring (P + Q)
                # We generate more tasks than needed because many fail XFOIL checks
                while len(offspring_tasks) < CONFIG['POPULATION_SIZE'] * 1.5:
                    p1 = self.crowded_tournament_selection()
                    p2 = self.crowded_tournament_selection()
                    
                    # SBX Crossover (Simulated Binary Crossover simplified)
                    alpha = random.random()
                ...
                if valid_offspring:
                    ...
                    # NSGA-II: Merge Parents + Offspring
                    ...
                    for f in fronts:
                        if len(next_gen) + len(f) <= CONFIG['POPULATION_SIZE']:
                            ...
                            f.sort(key=lambda x: x['distance'], reverse=True)
                            needed = CONFIG['POPULATION_SIZE'] - len(next_gen)
                            next_gen.extend(f[:needed])
                            break
                    self.population = next_gen
                    
                    # Reporting
                    best_ld = max(p['objectives'][0] for p in self.population)
                    best_width = max(p['objectives'][1] for p in self.population)
                    print(f"Gen {gen:03d} | Saved: +{len(valid_offspring):02d} | Total: {total_saved:04d} | "
                          f"Max L/D: {best_ld:.1f} | Max Width: {best_width:.3f} | Fronts: {len(fronts)}")
